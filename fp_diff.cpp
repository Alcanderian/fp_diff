...
static size_t fp_diff(const float *in0, const float *in1, const size_t &length, const float& eps_abs, const float& eps_rel, const float& eps_cos) {
    ...
    std::cerr << "COS distance: " << cos_distance << ", with SUM(ab)/N: " << sum_ab << ", SUM(a^2)/N:" << sum_a2 << ", SUM(b^2)/N: " << sum_b2 << std::endl;
...
int main(int argc, char** argv)
{
    ...
    size_t ret = fp_diff(buf0.get(), buf1.get(), std::min(len0, len1) / sizeof(float), 1e-5f, 1e-5f, 1e-2f);
    if (ret == 0) {
